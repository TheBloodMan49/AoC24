...
def number_of_trails(data: list[list[int]], pos: tuple[int,int], current: int, target: int) -> list[tuple[int,int]]:
    ...
    if pos[0] > 0 and data[pos[0]-1][pos[1]] == current+1:
        res = res + number_of_trails(data, (pos[0]-1, pos[1]), current+1, target)
    if pos[0] < len(data)-1 and data[pos[0]+1][pos[1]] == current+1:
        res = res + number_of_trails(data, (pos[0]+1, pos[1]), current+1, target)
    if pos[1] > 0 and data[pos[0]][pos[1]-1] == current+1:
        res = res + number_of_trails(data, (pos[0], pos[1]-1), current+1, target)
